          ...
          sections: [
            {
              ...
              steps: [
                ...
                { ref: 'Race track', name: '# bakjes', from: '75', to: '75', changed: false },
                { ref: 'Race track', name: '# bakjes', from: '12', to: '12', changed: false },
                { ref: 'Induwers', name: 'Pakjes', from: '12', to: '12', changed: false },
                { ref: 'Induwers', name: 'Pakjes', from: '6', to: '6', changed: false },
                { ref: 'Wisselen', name: 'Gripper', from: '6', to: '6', changed: false },
                { ref: 'Recept', name: 'Digestieve 170', from: '70', to: '72', changed: true },
                { ref: 'Recept', name: '3-D sensor', from: '2', to: '5', changed: true }
              ]
            },
            {
              id: 'dozen_opzetter',
            ...
            },
            {
              ...
              steps: [
                ...
                { ref: 'NR 8', name: 'Achterzijde', from: '501,4', to: '508,3', changed: true },
                { ref: 'NR 8', name: 'Transport geleiding deksel', from: '502,9', to: '500,3', changed: true },
...
function renderLines() {
  ...
  data.lines.forEach(line => {
    ...
    if (!line.active) button.style.opacity = '0.5';
...
function renderWizard() {
  const step = selectedSection.steps[wizardIndex];
  const progress = ((wizardIndex + 1) / selectedSection.steps.length) * 100;

  document.getElementById('wizardCounter').textContent = `Step ${wizardIndex + 1} / ${selectedSection.steps.length}`;
...
document.getElementById('wizardNextBtn').onclick = () => {
  if (wizardIndex < selectedSection.steps.length - 1) {
    wizardIndex++;
    renderWizard();
  } else {
    showScreen('screen-section-detail');
